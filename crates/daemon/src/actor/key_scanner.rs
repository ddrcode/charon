...
use maiko::{Actor, Context, Meta, Result as MaikoResult};
...
impl Actor<CharonError> for KeyScanner {
    ...
    async fn handle(&mut self, event: &DomainEvent, _meta: &Meta) -> MaikoResult<()> {
        match &event {
            DomainEvent::Exit => {
                self.ctx.stop();
            }
    ...
    async fn tick(&mut self) -> MaikoResult<()> {
